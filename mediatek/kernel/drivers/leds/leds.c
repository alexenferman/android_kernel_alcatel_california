...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		...
		{       
            ...
            #else
			upmu_set_isink_ch1_step(0x3); // add to 16mA by zhengdao
			
			#endif
            upmu_set_isink_sfstr1_tc(0x0); // 0.5us
            upmu_set_isink_sfstr1_en(0x0); // Disable soft start
			upmu_set_rg_isink1_double_en(0x1); // Disable double current ,double enable
			upmu_set_isink_phase1_dly_en(0x0); // Disable phase delay
            upmu_set_isink_chop1_en(0x0); // Disable CHOP clk
			led_init_flag[1] = true;
		}
...
static int mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	struct nled_setting led_tmp_setting = {0, 0, 0};
	int tmp_level = level;
//ting.kang add for close the BL when without LCD	
#if defined(JRD_BUILD_MMITEST)
	char *p,*q;
	char mtkfb_islcmfound[256] = {0};
#endif
//ting.kang end
...
//ting.kang add for close the BL when without LCD
#if defined(JRD_BUILD_MMITEST)
	p = strstr(saved_command_line, "islcmfound=");
	printk("mt65xx_leds_set_cust: P=%s \n",p);
	p += 11;
    q = p;
    while(*q != ' ' && *q != '\0')
        q++;
    
    strncpy((char*)mtkfb_islcmfound, (const char*)p, (int)(q-p));
    
    printk("%s, kangting mtkfb_islcmfound = %c\n", __func__, mtkfb_islcmfound[0]);
	
if (mtkfb_islcmfound[0]!='1')
{
	printk("%s, backlight is off = \n", __func__);

	mt_set_gpio_mode(GPIO30, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO30, GPIO_DIR_OUT); //output
