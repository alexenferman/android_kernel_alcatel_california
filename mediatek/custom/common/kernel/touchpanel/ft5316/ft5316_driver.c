...
static int ft5x0x_i2c_rxdata(char *rxdata, int length)
{
	...
	struct i2c_msg msgs[] = {
		{
			#ifdef __MSG_DMA_MODE__
			.addr	= this_client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG,
			.buf	= g_dma_buff_pa,
			#else
	...
	{
		memcpy(rxdata, g_dma_buff_va, length);
	}
	#endif
	return ret;
}
...
static int ft5x0x_i2c_txdata(char *txdata, int length)
{
	...
	if (g_dma_buff_va == NULL)
		return;
	memcpy(g_dma_buff_va, txdata, length);
#endif
...
static int ft5x0x_read_reg(u8 addr, u8 *pdata)
{
	...
	#ifdef __MSG_DMA_MODE__
	msgs[0].addr = this_client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
	msgs[0].buf = g_dma_buff_pa;
	#else
	...
	#ifdef __MSG_DMA_MODE__
	msgs[1].addr = this_client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
	msgs[1].buf = g_dma_buff_pa;
	#else
...
static void ft5x0x_read_reg_dma(u8 addr, u8* read_data, u16 size)
{
   	int rc;
	#ifdef __MSG_DMA_MODE__
	if (g_dma_buff_va == NULL)
	...
	if( rc < 0 )
    {
		printk("ft5x0x_read_reg error %d\n", rc);
	}
	#ifdef __MSG_DMA_MODE__
	else
	{
...
FTS_BOOL i2c_read_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	__u8 *buf;
		...
		buf = pbt_buf;
	#endif
    ret = i2c_master_recv(this_client, buf, dw_lenth);
    if(ret <= 0)
    {
...
FTS_BOOL i2c_write_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	memcpy(g_dma_buff_va, pbt_buf, dw_lenth);
		...
		buf = pbt_buf;
	#endif
    ret = i2c_master_send(this_client, buf, dw_lenth);
    if(ret<=0)
    {
...
static unsigned char CTPM_FW_CALIFORNIA_FT5316_ID5A[]=
{
	#include "California_ID5A_V1b_20130813_app.i"
};


static unsigned char CTPM_FW_CALIFORNIA_FT5316_V0A[]=
{
	#include "California_ID5A_V1b_20130813_app.i"
};

static unsigned char CTPM_FW_CALIFORNIA_FT5316_ID3B[]=
{
	#include "California_ID3B_V0d_20130814_app.i"
};
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    delay_qt_ms(40);     //added by zhengdao for stable
	#else        //solve tp update failure question

	//rst pin
		mt_set_gpio_mode(GPIO57, 0);
	...
	//	mt_set_gpio_dir(GPIO17, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO57, GPIO_OUT_ONE);
	delay_qt_ms(30);	 //added by zhengdao for stable


	
	#endif        //solve tp update failure question
...
//        ft5x0x_read_reg(0,&uc_temp);
		ft5x0x_read_reg_dma(0,&uc_temp[0],2);
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if (tp_vendor == 0x5A)
	{
		pbt_buf = CTPM_FW_CALIFORNIA_FT5316_ID5A;
		ui_sz = sizeof(CTPM_FW_CALIFORNIA_FT5316_ID5A);
	}
	else if (tp_vendor == 0x3B)
	{
		pbt_buf = CTPM_FW_CALIFORNIA_FT5316_ID3B;
		ui_sz = sizeof(CTPM_FW_CALIFORNIA_FT5316_ID3B);
	}
	else
		return 0xff;
   ...
   else
   {
     ...
     //  fts_ctpm_auto_clb();  //start auto CLB  ,cance for it have been executed
   }
   return i_ret;
}
...
unsigned char fts_ctpm_get_i_file_ver(void)
{
    ...
    if (tp_vendor == 0x5A)
    {
		...
		if (ui_sz > 2)
		{
			return CTPM_FW_CALIFORNIA_FT5316_ID5A[ui_sz - 2];
		}
		else
		//TBD, error handling?
    ...
    else if (tp_vendor == 0x3B)
    {
		...
		if (ui_sz > 2)
		{
			return CTPM_FW_CALIFORNIA_FT5316_ID3B[ui_sz - 2];
		}
		else
		//TBD, error handling?
		return 0xff; //default value
...
int fts_ctpm_auto_upg(void)
{
	...
	u8 data = 0xaa;
    unsigned char version_list_california_ft5316_ID5A[] = {0x0,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a};	//Keep the ft5316 tp firmware old version list that allows to be updated.
    unsigned char version_list_california_ft5316_ID3B[] = {0x0,0x0a,0x0b,0x0c};
    int i,i_ret = -1,ret;
	#ifdef __MSG_DMA_MODE__
	unsigned short addr;
	#endif
...
//	ft5x0x_read_reg(FT5X0X_REG_FT5316ID, &tp_vendor);
    ...
    ret = i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FT5316ID, 1, &tp_vendor);
    printk("mtk_tpd: tp vendor = 0x%x\n", tp_vendor);
	ret = i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FIRMID, 1, &uc_tp_fm_ver);
	...
	if((tp_vendor != 0x5a) && (tp_vendor != 0x3b))
	{
	...
	if(tp_vendor == 0x5a){
		...
		for(i = 0;i < sizeof(version_list_california_ft5316_ID5A)/sizeof(version_list_california_ft5316_ID5A[0]); i++)
		{
			if(uc_tp_fm_ver == version_list_california_ft5316_ID5A[i])
			{
		...
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		printk("[FT5316] tp vendor is BIEL(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		for(i = 0;i < sizeof(version_list_california_ft5316_ID3B)/sizeof(version_list_california_ft5316_ID3B[0]); i++)
		{
			if(uc_tp_fm_ver == version_list_california_ft5316_ID3B[i])
			{
				i_ret = fts_ctpm_fw_upgrade_with_i_file();
				
				if(!i_ret)
	     			printk("[FTS] BIEL tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
				else
	    			printk("[FTS] BIEL tp firmware upgrade failed ret=%d.\n", i_ret);
				break;
			}
		}
	}
	...
	if ((tp_vendor == 0x5a) || (tp_vendor == 0x3b)){
		i2c_smbus_read_i2c_block_data(this_client, 0x96, 1, &chip_calibration_value);

		printk("touchpanel chip_calibration_value is 0x%2x xxxxxxxxxxxx\n", chip_calibration_value);
		if (chip_calibration_value != 0xaa) {
		i2c_smbus_write_i2c_block_data(this_client, 0x96, 1, &data);
...
static ssize_t ft5316_show_version(struct device *dev,struct device_attribute *attr, char *buf)
{
	u8 uc_reg_value;
...
static ssize_t ft5316_store_updata(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	#ifdef __MSG_DMA_MODE__
	msg_dma_alloct();
	#endif
	...
	if(!mode){
	//=ven========FW upgrade========================*/
		pbt_buf = CTPM_FW_CALIFORNIA_FT5316_V0A;
		ui_sz = sizeof(CTPM_FW_CALIFORNIA_FT5316_V0A);
	/*call the upgrade function*/
		i_ret =  fts_ctpm_fw_upgrade(pbt_buf,ui_sz);
	}
	if (i_ret != 0)
...
static DRIVER_ATTR(updata,    S_IWUSR | S_IRUGO,   NULL,  ft5316_store_updata);
static DRIVER_ATTR(version,   S_IWUSR | S_IRUGO,   ft5316_show_version, NULL);
...
static int ft5x0x_GetFirmwareSize(char *firmware_name)
{
	...
	printk("[FT5316_llf] Get firmware size begain\n");
			       ...
			       unsigned char *firmware_buf)
{
	...
	printk("[FT5316_llf] Read firmware begain!\n");
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s", firmware_name);
	if (NULL == pfile)
		pfile = filp_open(filepath, O_RDONLY, 0);
				       ...
				       char *firmware_name)
{
	...
	printk("[FT5316_llf] Get firmware size ok size=%d \n",fwsize);
	if (fwsize <= 0) {
	...
	printk("[FT5316_llf] Get firmware file ok\n");

	printk("[FT5316_llf] File just is 0x%x 0x%x 0x%x \n",(pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]),
	...
	if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) == 0xFF
		...
		/*call the upgrade function */
		printk("[FT5316_llf] Begain to Upgrade firmware \n");
		...
		printk("[FT5316_llf] Firmware file format error\n");
		kfree(pbt_buf);
		return -EIO;
	}
	return i_ret;
...
static ssize_t ft5x0x_fwupgradeapp_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	printk("[FT5316_llf]Input file name is %s \n",fwname);



    ret = i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FT5316ID, 1, &tp_vendor);
    printk("mtk_tpd: tp vendor = 0x%x\n", tp_vendor);
	ret = i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FIRMID, 1, &fm_ver);
    printk("mtk_tpd: Firmware version = 0x%x\n", fm_ver);
...
//	printk("[FT5316_llf] current firmware version is  0x%x\n", fm_ver);
 //   ft5x0x_read_reg(0xA8, &tp_vendor);
	...
	if((tp_vendor != 0x5a)&&(tp_vendor != 0x3b)){
		...
		printk("[%s] can't find ft5316,vendor:0x%x\n",__func__,tp_vendor);
		return -1;
	}
	...
	printk("[FT5316_llf] Firmware Upgrade Successful!\n");

	fm_ver = ft5x0x_read_fw_ver();

	printk("[FT5316_llf]  After Upgrade firmware version is  0x%x\n", fm_ver);
...
reset_proc:   
  ...
  mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
	...
	msleep(10);		
    mt_set_gpio_out(GPIO57, GPIO_OUT_ZERO);
	msleep(5);	// for tp time stamp ,cannot be changed
    mt_set_gpio_out(GPIO57, GPIO_OUT_ONE);
	//msleep(200);     // for tp time stamp ,cannot be changed
	//enable
	//mt_set_gpio_mode(GPIO12, 0);
	//mt_set_gpio_dir(GPIO12, GPIO_DIR_OUT);
	//mt_set_gpio_out(GPIO12, GPIO_OUT_ONE);
    //mdelay(20);   // for tp time stamp ,cannot be changed
	//wake
	...
	msleep(320);  // for tp time stamp ,cannot be changed
	...
	else if (uc_reg_value == 0x3b)
	{
		printk("mtk_tpd: tp vendor is BIEL.\n");
	}
	else
		printk("mtk_tpd: tp vendor is 0x%x which is unknown.\n",uc_reg_value);
...
#endif
   mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
	 msleep(320); //added for tp time stamp ,cannot be changed 
...
static int __init tpd_driver_init(void) {
	...
	i2c_register_board_info(1, &ft5316_i2c_tpd, 1);
